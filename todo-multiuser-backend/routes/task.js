...
router.get('/assignedTo/:userId', auth, async (req, res) => {
  try {
    ...
    const tasks = await Task.find({ assignedTo: userIdObj }).populate('assignedBy', 'name email');
    res.json(tasks);
  } catch (err) {
    console.error(err);
    res.status(500).json({ message: 'Server error: ' + err.message });
...
router.get('/assignedBy/:userId', auth, async (req, res) => {
  try {
    ...
    const tasks = await Task.find({ assignedBy: req.params.userId }).populate('assignedTo', 'name email');
    res.json(tasks);
  } catch (err) {
    console.error(err);
    res.status(500).json({ message: 'Server error: ' + err.message });
...
router.patch('/:taskId', auth, async (req, res) => {
  try {
    ...
    const { title, description, company, assignedTo, priority, dueDate } = req.body;
    if (title !== undefined) task.title = title;
    if (description !== undefined) task.description = description;
    if (company !== undefined) task.company = company;
...
// Update task status (only assignee can update, supports completion remark and stuck reason)
router.patch('/:taskId/status', auth, async (req, res) => {
  try {
    ...
    // Save stuck reason if status is Stuck and stuckReason is provided
    if (req.body.status === 'Stuck') {
      task.stuckReason = req.body.stuckReason || '';
    } else {
      task.stuckReason = '';
    }
